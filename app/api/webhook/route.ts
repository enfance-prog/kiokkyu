...
import { parseNumbers } from "@/lib/numberParser";
...
const roomStates = new Map<
  string,
  {
    ...
    listName?: string;
    reminderName?: string;
    deleteTargets?: { type: "reminder" | "list"; ids: number[]; items: any[] };
    cleanupData?: any;
  }
>();
  ...
  for (const event of events) {
    ...
    if (event.type === "postback") {
      ...
      // クリーンアップ: すべて削除
        ...
        try {
          const result = await deleteStaleDataByIds(reminderIds, listIds);
          replyMessages = [
            {
              type: "text",
          ...
          replyMessages = [
            { type: "text", text: "クリーンアップでエラーが発生しちゃった😅" },
          ];
        }
      }
      // クリーンアップ: 選んで削除
      else if (action === "cleanup_select") {
        const reminderIds =
          data
            .get("reminder_ids")
            ?.split(",")
        ...
        try {
          const reminders = await getRemindersByIds(reminderIds);
          const lists = await getListsByIds(listIds);

          let text = "削除したいデータの番号を選んでね📝\n\n";
          let itemNumber = 1;
          const allItems = [];
          ...
          if (reminders.length > 0) {
            ...
            for (const reminder of reminders) {
              text += `  ${itemNumber}. ${reminder.message}\n`;
              allItems.push({
                number: itemNumber,
                id: reminder.id,
                type: "reminder",
          ...
          if (lists.length > 0) {
            ...
            for (const list of lists) {
              text += `  ${itemNumber}. ${list.list_name}\n`;
              allItems.push({ number: itemNumber, id: list.id, type: "list" });
              itemNumber++;
            }
            text += "\n";
          }
          ...
          text +=
            "例: 1 3 5\n\nキャンセルする場合は「キャンセル」って送ってね。";

          roomStates.set(roomId, {
            waitingFor: "cleanupNumbers",
            deleteTargets: {
              type: "reminder",
              ids: [],
              items: allItems,
          ...
          replyMessages = [
            { type: "text", text: "データの取得でエラーが発生しちゃった😅" },
          ];
        }
      }
      // クリーンアップ: 1ヶ月保留
      else if (action === "cleanup_postpone") {
        const reminderIds =
          data
            .get("reminder_ids")
            ?.split(",")
        ...
        try {
          await markCleanupWarning(reminderIds, listIds);
          replyMessages = [
            {
              type: "text",
              text: "⏰ 1ヶ月保留にしたよ\n\n1ヶ月後も使われていなかったら自動的に削除されるから、必要なデータは使っておいてね！",
            },
          ];
        } catch (error) {
          console.error("Cleanup postpone error:", error);
          replyMessages = [
            { type: "text", text: "保留設定でエラーが発生しちゃった😅" },
          ];
        }
      }
...
async function processMessage(roomId: string, message: string): Promise<any[]> {
  ...
  // 削除番号の入力待ち
  if (roomState?.waitingFor === "deleteNumbers") {
    if (message === "キャンセル") {
      roomStates.delete(roomId);
      return [
        {
    ...
    const deleteTargets = roomState.deleteTargets!;
    const validItems = deleteTargets.items.filter((item) =>
    ...
    if (validItems.length === 0) {
      return [
        {
          type: "text",
          text: "指定された番号が見つからなかったよ🤔\nもう一度確認してね！",
        },
      ];
    }

    try {
      const idsToDelete = validItems.map((item) => item.id);
      const deletedCount = await deleteRemindersByIds(roomId, idsToDelete);
      ...
      return [
        {
          type: "text",
          text: `${deletedCount}件のリマインダーを削除したよ🗑️\n\n削除した番号: ${numbers.join(
            ", "
          )}`,
        },
      ];
  ...
  // クリーンアップ番号の入力待ち
  if (roomState?.waitingFor === "cleanupNumbers") {
    if (message === "キャンセル") {
      roomStates.delete(roomId);
      return [
        {
          type: "text",
          text: "クリーンアップをキャンセルしたよ👍",
        },
      ];
    }
    ...
    try {
      const reminderIds = validItems
        .filter((item) => item.type === "reminder")
        .map((item) => item.id);
      const listIds = validItems
...
async function processReminderCommand(
  ...
  if (parts.length === 2 && parts[1] === "一覧") {
    try {
      const categorized = await getCategorizedReminders(roomId);
      ...
      if (categorized.active.length > 0) {
        text += "【未完了】\n";
        for (const reminder of categorized.active) {
          text += `  ${itemNumber}. ${reminder.message}\n`;
          text += `     (${formatDateTime(new Date(reminder.remind_at))})\n`;
          allItems.push({
            number: itemNumber,
            id: reminder.id,
            type: "active",
      ...
      if (categorized.pending.length > 0) {
        text += "【期限超過】⚠️\n";
        for (const reminder of categorized.pending) {
          text += `  ${itemNumber}. ${reminder.message}\n`;
          text += `     (${formatDateTime(new Date(reminder.remind_at))})\n`;
          allItems.push({
            number: itemNumber,
      ...
      if (categorized.completed.length > 0) {
        text += "【完了済み】✅\n";
        for (const reminder of categorized.completed) {
  ...
  // 「おしえてくん 削除」
  if (parts.length === 2 && parts[1] === "削除") {
    const roomState = roomStates.get(roomId);

    if (
      !roomState ||
      !roomState.deleteTargets ||
      roomState.deleteTargets.items.length === 0
    ) {
      return [
        {
          type: "text",
          text: "まず「おしえてくん 一覧」で削除したいリマインダーを表示してね！",
        },
      ];
    }
    ...
    roomStates.set(roomId, {
      ...roomState,
      waitingFor: "deleteNumbers",
    });

    return [
      {
        type: "text",
        text: "削除したいリマインダーの番号を入力してね📝\n\n例:\n  1 3 5\n  1, 3, 5\n  1. 3. 5.\n\nどの形式でもOKだよ！\n複数選択もできるよ👍\n\nキャンセルする場合は「キャンセル」って送ってね。",
      },
    ];
  }
